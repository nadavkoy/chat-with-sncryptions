...
class ClientHandler(threading.Thread):
    ...
    def handle_user_entrance(self, entrance_details):
        ...
        if entrance_type == 'register':  # if the client chose to register (connecting as a new user)
            ...
            self.sock.send(self.encrypt_message((SUCCESSFUL_ENTRY + '-' + MESSAGES)).ljust(1024, '0'))
            ...
            if self.database.user_exists(client_username, client_password):
                ...
                self.sock.send(self.encrypt_message(WRONG_DETAILS).ljust(1024, '0'))
    ...
    def send_messages(self, message, message_type):
        ...
        if message_type == BROADCAST_MESSAGE:
            for user_socket in CONNECTED_USERS.values():  # sending message to all the connected users.
                user_socket.send(self.encrypt_message(message).ljust(1024, '0'))
            ...
            if found_user:
                ...
                CONNECTED_USERS[send_to].send(
                    self.encrypt_message(message_type + sender + ':' + the_message).ljust(1024, '0'))
                CONNECTED_USERS[sender].send(
                    self.encrypt_message(message_type + sender + ':' + the_message).ljust(1024, '0'))

            if found_user is False:
                # if addressee is not connected, inform the sender
                CONNECTED_USERS[sender].send(self.encrypt_message(USER_NOT_FOUND).ljust(1024, '0'))
    ...
    def run(self):
        ...
        while True:
            ...
            message_from_client = self.decrypt_message(message_from_client).split('0')[0]
