...
def main():
    ...
    while True:
        ...
        client_hand = chat_client_handler.ClientHandler(address, socket, server.public_key, server.private_key, server.rsa, Database())
        client_hand.start()
        print 'chat server thread: ' + str(threading.current_thread().ident)
